...
1. 偏向锁
    ...
    1. 偏向锁的撤销
        ...
        偏向锁使用一种等竞争出现才释放锁的机制。当测试当前对象头为偏向锁且通过CAS将线程ID设置为当前访问的线程失败时，说明由多个线程竞争锁，此时需要撤销偏向锁。偏向锁的撤销需要等待全局安全点（此时没有正在执行的字节码），先暂停拥有偏向锁的进程，然后将对象头以及栈中的锁记录恢复为无锁状态并标记需要升级，然后唤醒该线程进行偏向锁的升级
...
2. 轻量级锁
    1. 轻量级锁加锁
    现在栈帧中创建锁记录，并将对象头中的mark word复制到锁记录中，官方称为 Displaced Mark Word 。然后线程使用CAS替换markword为指向锁记录的指针，如果成功则当前线程获得锁，如果失败，则其他线程正在竞争，便尝试使用自旋来获取锁
    2. 轻量级锁解锁
    当同时竞争线程过多，或者竞争线程自旋次数过多时，会发生锁的升级，升级为重量所（Monitor对象实现），此时，竞争线程将锁对象头markdown修改为指向重量级锁的指针（Monitor对象）。原持有锁的线程在使用CAS操作将Displaced Mark Word替换回对象头时，如果失败则发生锁的升级


![锁升级过程](../../resource/lockupdate.png)
    
### 锁的优缺点对比

| 锁 | 优点 | 缺点 | 适用场景 |
| :-----: | :----: | :----: | :----: |
| 偏向锁 | 加锁解锁不需要额外消耗 | 如果发生线程的竞争，撤销锁发生额外的资源浪费 | 只有一个线程访问同步块 |
| 轻量级锁 | 竞争线程不会阻塞 | 如果始终得不到锁，自旋会消耗CPU资源 | 少量线程竞争，追求响应时间 |
| 重量级锁 | 线程竞争不使用自旋，不消耗CPU | 线程阻塞，响应慢，发生上下文切换 | 锁竞争时间长，追求吞吐量 |
    
## 原子操作的实现原理
>　原子性：即一个操作或者多个操作 要么全部执行并且执行的过程不会被任何因素打断，要么就都不执行。
>
>   原子操作：不会被线程调度机制打断的操作，这种操作一旦开始，就一直运行到结束

### 处理器的实现方法
1. 通过总线锁保证原子性。当某一个处理器使用总线锁时，其他处理器访问内存的请求会被阻塞，该处理器独占内存
2. 通过缓存锁保证原子性。在将CPU的缓存数据写回主存时，修改内部的内存地址，并允许缓存一致性机制来保证操作的原子性，因为缓存一致性会阻止同时修改两个以上处理器缓存的内存区域数据，当其他处理器回写已被锁定的缓存行数据时，会使缓存行无效。

### Java的原子操作实现方法
1. 循环CAS来实现原子操作
Java中的CAS操作使用了系统提供的CMPXCHG指令。自旋CAS实现的思路就是循环进行CAS操作直到成功为止

2. CAS实现原子操作的三大缺点
    1. ABA问题。CAS操作值时，检查值是否发生变化，但如果一个值一开始是A，后来改为B，又改回A，那么CAS检查就会出现问题。解决思路除了比较值以外，在额外添加一个版本号的比较。JDK的AtomicStampedReference就是用来解决ABA问题
    2. 循环时间长开销大。
    3. 只能保证一个共享变量的原子操作。虽然只能保证一个变量的原子操作，但是可以把多个变量合成一个变量来实现多个变量的原子操作。
3. 使用锁机制实现原子操作
    锁机制保证只有获得锁的线程才能操作指定的内存区域。除了偏向锁，JVM实现锁的方式都用了循环CAS,当一个线程进入同步块通过CAS获得锁，当一个线程离开同步块通过CAS释放锁
